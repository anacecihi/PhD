...
# Save output
List = ["mono_multiple_low",str(rp),str(rx),str(av_sum_prey),str(av_sum_predator),str(av_sum_parasite),str(av_non_inv_abundance),str(av_inv_abundance),str(av_non_inv_rel_abundance),str(av_inv_rel_abundance),str(av_effective_prey),str(av_effective_predator),str(av_effective_parasite),str(av_effective_non_invader),str(av_effective_invader),str(av_lifespan_prey),str(av_lifespan_predator),str(av_lifespan_parasite),str(av_lifespan_non_invasive),str(av_lifespan_invasive),str(av_establishment_rate),str(invasion_success)]
List2 = ["mono_multiple_low",str(rp),str(rx),str(persistance_parasite),str(persistance_non_invaders),str(persistance_invaders),str(persistance_both),str(coexistence),str(coexistence_predator_and_prey),str(coexistence_parasite_and_prey),str(coexistence_prey),str(extinction)]

# Open our existing CSV file in append mode
# Create a file object for this file
if store_sum_prey != []:
    with open("output_mono_multiple_low.csv", 'a') as f_object:
        writer_object = writer(f_object)
        writer_object.writerow(List)
        f_object.close()

with open("coexistence_mono_multiple_low.csv", 'a') as f_object:
    writer_object = writer(f_object)
