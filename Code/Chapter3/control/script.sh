...
for rp in $(seq 0.5 0.1 1); do for rx in $(seq 0.5 0.1 1); do python $script_python/control.py $rp $rx $SGE_TASK_ID; done; done
...
python $script_python/average_files_control.py # run "final.py" which is inside "script_python"
